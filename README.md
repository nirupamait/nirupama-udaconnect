...
$Install apparmour using following::
Zypper in -t pattern apparmour
...
### Steps for deploying and running micro-services based architecture.The modules folder will have following microservices:
a)Person b)Connection c)Location



Steps for downloading and installing kafka in Kubernetes:::
Run following commands in sequence::
a)helm repo add bitnami https://charts.bitnami.com/bitnami
b)helm install my-release bitnami/kafka

We will get below o/p:

NAME: kafka-release
LAST DEPLOYED: Nov 07 19:33:16 2021
NAMESPACE: default
STATUS: deployed
REVISION: 1
TEST SUITE: None
NOTES:
 ...
 We can check Kafka pods in kubernetes using below command
...
Resource::https://bitnami.com/stack/kafka/helm


 PERSON API(Person Microservice) deployment and launching steps::
1. Go to the 'person-microservice' folder inside modules folder and run `$ kubectl apply -f deployment/.’
2.Check the deployment and pods status using below command::
Kubectl get pods.
3.When pods are running and ready, then execute database script using below command::
“sh /scripts/run_db_command.sh <POSTGRES_DB_POD_NAME”
4. Launch person api using the http://localhost:30001/api/persons .

CONNECTION API(Connection Microservice) deployment and launching steps::
1. Go to the 'connection-microservice' folder inside modules folder and run `$ kubectl apply -f deployment/.’
2.Check the deployment and pods status using below command::
Kubectl get pods.
3.When pods are running and ready, then execute database script using below command::
“sh /scripts/run_db_command.sh <POSTGRES_DB_POD_NAME”
4. Launch person api using the`http://localhost:30002/api/persons/600/connection?start_date=2020-01-01&end_date=2020-12-30&distance= .


LOCATION-EVENT-MICROSERVICE
1. Go into the 'location-microservice' folder and run `$ kubectl apply -f deployment/

FRONTEND
1. Go into the frontend folder and run `$ kubectl apply -f deployment/

When all pods are up and running, we will be able to launch:: the http://localhost:30000/
